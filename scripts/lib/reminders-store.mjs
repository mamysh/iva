...
export function resolveDueAt({ dueAt, dueAtLocal, dueInMinutes, dueInSeconds, timezone = DEFAULT_TIMEZONE }) {
  if (dueInSeconds !== undefined && dueInSeconds !== null) {
    ...
    if (!Number.isFinite(n) || n < 0) throw new Error("dueInSeconds должен быть неотрицательным числом");
    return new Date(Date.now() + Math.round(n * 1000));
  }
  if (dueInMinutes !== undefined && dueInMinutes !== null) {
    const n = Number(dueInMinutes);
    if (!Number.isFinite(n) || n < 0) throw new Error("dueInMinutes должен быть неотрицательным числом");
    return new Date(Date.now() + Math.round(n * 60_000));
  }
  if (dueAtLocal) return zonedLocalToUtc(String(dueAtLocal), timezone);
  if (dueAt) {
    const d = new Date(dueAt);
    if (Number.isNaN(d.getTime())) throw new Error("dueAt должен быть ISO-датой");
    return d;
  }
  throw new Error("Нужно указать dueAt, dueAtLocal, dueInMinutes или dueInSeconds");
}
...
export function createReminder(input, existing = []) {
  ...
  if (!reminder.text) throw new Error("text не должен быть пустым");
  if (reminder.repeatUntil && Number.isNaN(new Date(reminder.repeatUntil).getTime())) {
    throw new Error("repeatUntil должен быть ISO-датой");
  }
  return reminder;
}
...
export function formatReminder(reminder, { now = new Date(), timezone = reminder.timezone || DEFAULT_TIMEZONE } = {}) {
  ...
  const rel = due.getTime() <= now.getTime() ? "сейчас/просрочено" : formatLocal(due, timezone);
  const repeat = reminder.repeat && reminder.repeat !== "none" ? `, повтор: ${reminder.repeat}/${reminder.repeatInterval}` : "";
...
function zonedLocalToUtc(local, timezone) {
  ...
  if (!m) throw new Error("dueAtLocal должен быть YYYY-MM-DDTHH:mm[:ss]");
  ...
  if (Number.isNaN(out.getTime())) throw new Error(`Не удалось разобрать дату в timezone ${timezone}`);
  return out;
}
