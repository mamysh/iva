...
function fakeGit({ local = "local", remote = "remote", behind = "0", localVer = "0.2.5", remoteVer = "0.2.5", fail } = {}) {
  return async (...args) => {
    const key = args.join(" ");
...
assert.equal((await checkDeploymentUpdate(fakeGit({ local: "ahead", remote: "old", behind: "0" }))).hasUpdate, false);
