...
| `iva-update-check.timer` | daily at 10:00 in `ASSISTANT_TIMEZONE` | opt-in read-only comparison of the pinned `origin/stable` or `origin/main` channel and one Telegram offer per target commit |
...
channel selected at installation (`origin/stable` or `origin/main`), stores a private SHA dedup record,
and sends **View / Update / Later** buttons.
