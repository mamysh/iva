...
6. Добавить runtime env для PostgreSQL, worker concurrency и pool size.
   Обязательно задать `WORKFLOW_QUEUE_NAMESPACE=eve`: generated workflow routes Eve
   скомпилированы с этим namespace. Без него Graphile Worker получает `Unhandled queue`.
   `WORKFLOW_POSTGRES_JOB_PREFIX` заканчивается подчёркиванием (`iva_`) и используется
   только для читаемых имён Graphile task types.
...
Дополнительный staging-инцидент 2026-06-27: Eve 0.11.10 не завершила HTTP/workflow handles
по SIGTERM за 10 секунд. Для предсказуемого deploy systemd получает
`TimeoutStopSec=15s`, `SendSIGKILL=yes` и `RestartSec=2s`; это не влияет на checkpointed
сессию, подтверждённую продолжением после SIGKILL.
