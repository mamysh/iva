...
- Память/время: `ASSISTANT_TIMEZONE` (IANA, дефолт `Europe/Minsk` — даты транскрипта, таймеры,
...
## Веб-поиск: выбор провайдера
Инструмент `web_search` ходит к выбранному API-провайдеру (`SEARCH_PROVIDER`), если задан его ключ.
Если ключ пустой, Iva использует DuckDuckGo fallback: поиск работает сразу, но API-провайдеры стабильнее
для VPS. При лимите, отклонённом ключе или неожиданном ответе API один запрос тоже переключается на
DuckDuckGo fallback, а в результате tool появляется заметка с причиной. `iva config` проводит по выбору.

| Провайдер | `SEARCH_PROVIDER` | Ключ env | Стоимость/лимит | Карта | `answer` |
|-----------|-------------------|----------|-----------------|:-----:|:--------:|
| Tavily ★  | `tavily`   | `TAVILY_API_KEY`   | free ~1000/мес            | нет | да |
| Exa       | `exa`      | `EXA_API_KEY`      | free ~20k/мес             | нет | нет |
| Parallel  | `parallel` | `PARALLEL_API_KEY` | стартовые кредиты, $5/1k  | нет | нет |
| Brave     | `brave`    | `BRAVE_API_KEY`    | ~$5/мес кредит (условия меняются — проверяй актуальные) | **да** | нет |

- Дефолт — **Tavily** (без карты, единственный возвращает готовый `answer`).
- Только **Brave** требует привязку карты (идентификация). Inline-`answer` есть только у Tavily;
  у Exa/Parallel/Brave Iva собирает контекст из сниппетов сама.
- Ключ можно сменить в `.env` без пересборки (перезапусти процесс). Несколько ключей могут
  сосуществовать — переключение только флагом `SEARCH_PROVIDER`. Без ключа активного провайдера
  или при лимите/сбое API поиск переходит на DuckDuckGo fallback с явной заметкой.
